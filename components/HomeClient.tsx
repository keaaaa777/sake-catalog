...
import { HomeSakeSummary, REGIONS, PREFECTURE_SLUGS } from '@/lib/types'
...
export default function HomeClient() {
  ...
  const loadSakes = async () => {
    if (sakes.length > 0 || isLoadingSakes) return
    setIsLoadingSakes(true)
    setSakesLoadError(false)
    try {
      ...
      setSakesLoadError(true)
    } finally {
      setIsLoadingSakes(false)
    }
  }
  ...
  const filteredSakes = useMemo(
    () =>
      sakes.filter((sake) =>
        sake.searchText.includes(searchQuery.normalize('NFKC').toLocaleLowerCase('ja-JP'))
      ),
    [sakes, searchQuery]
  )
        ...
        <section className="search-selector">
          ...
          <Link
            ...
            onClick={(e) => {
              e.preventDefault()
              void loadSakes()
              changeStepWithTransition('home', 'panel-brand')
            }}
          >
            <div className="option-btn__content">
              <div className="option-btn__meta">
        ...
        {activePanel === 'panel-brand' && (
          <div className="sake-panel is-active">
            ...
            <div className="mb-6">
              <input
                type="text"
                placeholder="銘柄名、酒蔵、都道府県、分類などで検索..."
                value={searchQuery}
            ...
            <div className="brand-list max-h-[350px] overflow-y-auto pr-2">
              {isLoadingSakes ? (
                <div className="text-center py-6 text-sm text-washi/40" role="status">銘柄データを読み込んでいます...</div>
              ) : sakesLoadError ? (
                <div className="text-center py-6 text-sm text-washi/50">
                  <p>銘柄データを読み込めませんでした</p>
                  <button type="button" onClick={() => void loadSakes()} className="mt-3 text-gold hover:underline">再読み込み</button>
                </div>
              ...
              sakes.filter(sake => favorites.includes(sake.id)).map(sake => (
                ...
                >
                  <Link href={`/sake/${sake.slug}`} className="flex items-center gap-3">
                    <span
                      className="h-10 w-10 shrink-0 rounded-lg border border-gold/20 bg-cover bg-center"
                      style={{ backgroundImage: `url(${JSON.stringify(sake.backgroundImageUrl)})` }}
                      aria-hidden="true"
                    />
                    <div>
