...
### 信頼性向上(出典調査)の進捗(2026年9月7日時点・優先度3完了)
...
5.その他)で公式出典の調査・登録を実施し、優先度5「その他」の残り
140蔵元(未出典銘柄281件)についても調査を完了した。

- **銘柄の出典登録率: 約94.6%**(54/997件が未登録、943件登録済み。
  優先度5着手前は72%・281件未登録)
- **蔵元の出典登録率: 約93.8%**(19/307件が未登録、288件登録済み。
  優先度5着手前は54%・140件未登録)
...
- 調査手法: `data/sakes.json`の`breweryId`(=`data/breweries.json`の
  ...
  蔵元から優先的に調査対象を抽出。今回は140蔵元を4分割し、並行動作する
  調査エージェント(WebSearch/WebFetch使用、データ編集権限なし)に
  それぞれ公式サイト調査を委任し、結果をレビューした上で
  `sources: [{title, url, type: 'official-brewery', checkedAt}]` /
  `verificationStatus: 'supported'` / `lastReviewedAt`を蔵元・配下銘柄
  双方に一括反映するワンオフスクリプトで適用。反映後は
  `npm run validate-data`でデータ整合性を確認し、新規追加した
  公式URL121件についてもHTTPアクセス確認(User-Agent付き)を実施した
  (一部サイトはボット対策で直接確認できなかったが、調査エージェントが
  WebFetchで内容一致を確認済み)
- 検証ポリシー: 公式サイトで蔵元・銘柄の実在を確認できた場合のみ
  `'supported'`を付与し、`'verified'`(数値スペックまで一致確認)は
  使用していない(規格値は分類ベースの推定値のままのため)
- 公式サイトが確認できず未着手のまま残った蔵元(19件、再調査候補):
  墨廼江酒造(宮城)・丸尾本店(香川)・タカハシ酒造(三重)・アリサワ(高知)・
  越銘醸(新潟)・呉春(大阪)・玄葉本店(福島)・合同酒精(北海道、現在は
  オエノングループのブランドで独自ドメインなし)・青島酒造(静岡)・
  亀の井酒造(山形。大分県に同名の別会社があり要注意)・秋鹿酒造(大阪)・
  廣木酒造本店(福島)・長珍酒造(愛知、公式サイトなし・通販もなしと確認)・
  樋木酒造(新潟)・山村酒造(熊本)・萬乗醸造(愛知、醸し人九平次の蔵元。
  公式サイトなしと明言する情報源あり)・三宅彦右衛門酒造(福井)・
  川村酒造店(岩手)、および高木酒造(山形、公式サイトを意図的に
  持たないことを確認済み)
